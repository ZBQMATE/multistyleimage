...
def main():
    ...
    ic = int(math.log(datasty.size / datacon.size, 4))
    ccc = []
    iii = []
    for i in range(5 - ic):
        iii.append(compute_style[i])
        ccc.append(compute_style[i + ic])
    ...
    #        temp_style = np.reshape(temp_style, (-1, temp_style.shape[3]))
    #        infostyb[layer] = np.matmul(temp_style.T, temp_style) / temp_style.size
    # read from data, should change style input picture size manually
    for layer in compute_style:
        bb = np.load(layer + ".npy")
        infostyb[layer] = bb
    ...
    with g.as_default(), tf.Session() as sess:
        ...
        for layer in range(5 - ic):
            ...
            temp_center = center_vgg[iii[layer]]
            #_, height, width, number = map(lambda i: i.value, temp_center.get_shape())
            _, height, width, number = map(lambda i: i, temp_center.get_shape())
            ...
            style_gram = infosty[ccc[layer]]
            style_gramb = infostyb[ccc[layer]]
            ...
            uuu = tf.nn.l2_loss(center_gram * 2 - style_gram * 10 / 7 - style_gramb * 4 / 7) / style_gram.size
            losssty.append(uuu)
